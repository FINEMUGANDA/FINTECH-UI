...
angular.module('angularjsApp').controller('CostCenterCtrl', function($scope, REST_URL, CostCenterService, $timeout, $location, dialogs) {
  ...
  $scope.removeCostCenter = function(costCenter) {
    var msg = 'You are about to remove Cost Center <strong>' + costCenter.staff.displayName + '</strong>';
    ...
    dialog.result.then(function(result) {
      if (result) {
        ...
        CostCenterService.removeCostCenter(REST_URL.COST_CENTER + costCenter.staff.id).then(function() {
          if (index >= -1) {
...
//      console.log(result.data.glAccounts);
  ...
  var loadCostCenter = function getData() {
    ...
    $timeout(
      function() {
        ...
        if ($scope.staffId) {
          url = REST_URL.COST_CENTER + $scope.staffId + '?template=true';
        }
        //service to get accounts from server
...
//        if (!$scope.staffId) {
//          loadCostCenter();// load updatedData
//        } else {
//          $scope.staffId = result.data.resourceId;
//        }
      };
